...
export function treeAppearance(surah,percent=100){const stage=treeStage(percent),n=surah-1,family=TREE_FAMILIES[n%TREE_FAMILIES.length],variant=Math.floor(n/TREE_FAMILIES.length)%TREE_VARIANTS.length,cycle=Math.floor(n/50),look=TREE_VARIANTS[variant],positions=[0,13,32,59,90],parcelPositions=[0,13,32,62,100],parcelWidths=[52,80,80,86,92],zooms=[180,160,120,100,100];return{...stage,family,variant,identity:(n%50)+1,hue:look.hue+cycle*4,shape:look.shape+(cycle%3)*.012,position:positions[stage.index],parcelPosition:parcelPositions[stage.index],parcelWidth:parcelWidths[stage.index],zoom:zooms[stage.index]}}
