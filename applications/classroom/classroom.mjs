...
import {queueTreeMoment,nextTreeMoment,hasUnreadTreeMessage} from './moments.mjs';
export async function mountClassroom(root,profile,options={}){
 ...
 root.dataset.live=String(Boolean(cloud));
 ...
 const previewUrl=(c,s)=>{const url=location.pathname.endsWith('/demonstration.html')?new URL('demonstration.html',location.href):new URL('../../Admin-Classe-Coran-Essai.html',import.meta.url);url.search='';url.hash='';url.searchParams.set('preview','1');if(new URLSearchParams(location.search).get('qa')==='motion')url.searchParams.set('qa','motion');url.searchParams.set('class-id',c.id);url.searchParams.set('student-id',s.id);return url.href};
 ...
 function syncStatus(){
  ...
  host.innerHTML=saveFailure?`<span>⚠ ${esc(saveFailure)}</span><button type="button" data-action="refresh-cloud">Actualiser les données</button>`:pendingWrites?'Enregistrement en cours…':cloud?'✓ Enregistré en ligne':'';
 ...
 host.innerHTML+=`${!studentView?teacherInbox():nextStep()}<p class="cc-note">${studentView?'Touche un arbre pour retrouver ton passage et les messages de ton professeur.':'Vue professeur : touchez un arbre pour valider et écrire votre appréciation.'}</p><div class="cc-row"><label>Ma parcelle<select id="cc-parcel">${JUZ_NAMES.slice(1).map((n,i)=>`<option value="${i+1}" ${parcel===i+1?'selected':''}>Juz’ ${i+1} · ${esc(n)}</option>`).join('')}</select></label></div><p>${done} / ${total} versets validés dans ce Juz’</p><progress value="${done}" max="${total}" aria-label="Avancement du Juz’"></progress><div class="cc-tree-list">${visible.map(s=>`<button class="cc-tree-option" data-action="tree" data-id="${s.id}">${art(s.id)}<strong>${esc(s.name)}</strong><small>${tree(s.id).verses.length}/${s.count} validés ${(tree(s.id).pendingVerses||[]).length?` · ${(tree(s.id).pendingVerses||[]).length} en attente`:``} ${due(tree(s.id))?'💧':''} ${hasUnreadTreeMessage(tree(s.id))?'<span class="cc-unread" aria-label="Nouveau message du professeur">✉️</span>':''}</small></button>`).join('')||'<p>Cette parcelle attend ses premières sourates. Tu peux toujours explorer le Coran.</p>'}</div><div class="cc-board" aria-label="Jardin personnel">${visible.filter(s=>tree(s.id).positions[parcel]).map(s=>{const pos=tree(s.id).positions[parcel];return `<button class="cc-planted ${growthMotion(s.id)&&(bloom.colorReveal||bloom.complete)?'cc-celebrate':''}" style="left:${pos.x}%;top:${pos.y}%" data-action="tree" data-id="${s.id}"><span class="cc-tree-crown">${art(s.id)}</span><span>${esc(s.name)} ${due(tree(s.id))?'💧':''} ${hasUnreadTreeMessage(tree(s.id))?'<b class="cc-unread" aria-label="Nouveau message du professeur">✉️</b>':''}</span></button>`}).join('')}</div><div id="cc-detail"></div>`;
 ...
 function detail(){const s=chapter(selected),t=tree(selected),host=root.querySelector('#cc-detail');if(!host)return;
  if(studentView&&hasUnreadTreeMessage(t)){t.messagesSeenAt=new Date().toISOString();save();root.querySelectorAll(`[data-action="tree"][data-id="${selected}"] .cc-unread`).forEach(badge=>badge.remove())}
 ...
 const changingSpace=['classes','class','roster','student','toggle','tree','close','next-student','previous-student','inbox-student','next-practice','record-passage','moment-tree'].includes(a)||a.startsWith('tab-');
 ...
 if(cloud)refreshTimer=setInterval(()=>refreshCloud(),20000);
 ...
 window.addEventListener('beforeunload',e=>{if(pendingWrites||formBusy||recordingSession||draftBlob||verseNoteSession||verseNoteBlob){e.preventDefault();e.returnValue=''}});
 window.addEventListener('pagehide',()=>{clearInterval(refreshTimer);stop();motion.destroy()},{once:true});draw();
}
