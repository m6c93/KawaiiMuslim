...
export async function mountClassroom(root,profile,options={}){
 ...
 const gardenArrange=createGardenArrange(root,{
  canMove:()=>parcel>0&&Boolean(studentId)&&!pendingWrites&&!formBusy&&!saveFailure&&root.dataset.expired!=='true',
 ...
 if(parcel===0){const board=root.querySelector('.cc-board'),columns=host.clientWidth<600?2:3,rows=Math.ceil(visible.length/columns),height=Math.max(480,rows*170+160);board.style.height=height+'px';board.innerHTML=visible.map((s,i)=>{const x=100*(i%columns+1)/(columns+1),y=(190+Math.floor(i/columns)*170)/height*100,unread=hasUnreadTreeMessage(tree(s.id));return `<button class="cc-planted ${unread?'cc-has-message':''}" style="left:${x}%;top:${y}%" data-action="tree" data-id="${s.id}"><span class="cc-tree-crown">${art(s.id)}</span><span>${esc(s.name)} ${due(tree(s.id))?'💧':''}</span>${messageBadge(s.id,true)}</button>`}).join('')||'<p class="cc-note">Les arbres apparaîtront ici dès que ton professeur aura validé des versets.</p>';root.querySelector('.cc-tree-list').style.display='none';}
