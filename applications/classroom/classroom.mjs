...
export async function mountClassroom(root,profile,options={}){
 ...
 function initialParcel(c){return 0}
 ...
 async function sendRecording(){
  ...
  state.phase='sending';state.errorCode='';notice='';root.querySelector('.cc-status').textContent='';state.message='Je dépose ton enregistrement dans la classe…';renderRecitationWorkspace();
 ...
 function updateShell(){
  ...
  if(!studentView&&!cloud?.studentId&&!root.querySelector('[data-action="tab-teacher-coran"]'))root.querySelector('#cc-navigation').insertAdjacentHTML('afterbegin',`<nav class="cc-row" aria-label="Espaces du professeur">${button(classId?'Ma classe':'Mes classes','tab-teacher-class')}${button('📖 Coran','tab-teacher-coran')}</nav>`);
  root.querySelector('[data-action="tab-teacher-coran"]')?.setAttribute('aria-current',studentSection==='teacher-coran'?'page':'false');
 ...
 async function sendTeacherPassage({classId:targetId,ids,passage,kind,note,audio}){
  if(studentView||cloud?.studentId||formBusy||saveFailure)throw Error('Envoi indisponible. Actualisez puis réessayez.');
  const target=db.classes.find(c=>c.id===targetId),s=chapter(passage.id);
  if(!target||!s||!ids.length||ids.some(id=>!target.students.some(p=>p.id===id)))throw Error('Choisissez les élèves de votre classe.');
  const label=kind==='learn'?'À apprendre':'À réviser',instruction=label+' · '+String(note||'').trim();
  assignReview(emptyTree(),passage.from,passage.to,s.count,teacher,instruction);
  formBusy=true;
  try{
   const audioIds=new Map();
   if(audio)for(const id of ids){const audioId='verse-comment-'+crypto.randomUUID();if(cloud)await cloud.audioSave(targetId,id,audioId,audio);else await saveRecording(audioId,audio);audioIds.set(id,audioId)}
   target.surahs||=[];
   if(!target.surahs.includes(s.id)&&!JUZ_RANGES.find(r=>r.surah===s.id)?.ranges.some(r=>target.juz.includes(r.juz)))target.surahs.push(s.id);
   for(const id of ids){const p=target.students.find(p=>p.id===id),t=p.trees[s.id]||=(emptyTree());assignReview(t,passage.from,passage.to,s.count,teacher,instruction);t.assignment.kind=kind;t.events.at(-1).kindLabel=label;
    if(audioIds.has(id)){t.verseComments||=[];t.verseComments.unshift({id:crypto.randomUUID(),verse:passage.from,text:instruction,audioId:audioIds.get(id),author:teacher,at:new Date().toISOString()})}
   }
   if(!await save())throw Error('L’envoi n’a pas été confirmé. Actualisez les données avant de réessayer.');
 ...
 if(!studentView&&studentSection==='teacher-coran'){mountTeacherCoran(host,{index,classes:db.classes,currentClass:classId,currentStudent:studentId,esc,practice,getPassage:()=>practicePassage,send:sendTeacherPassage});return}
 ...
 const all=allowed(),visible=all.filter(s=>parcel===0?tree(s.id).verses.length>0:JUZ_RANGES.find(r=>r.surah===s.id).ranges.some(r=>r.juz===parcel)).sort((a,b)=>priority(b)-priority(a));
 const ranges=parcel===0?visible.map(s=>({surah:s.id,from:1,to:s.count})):JUZ_RANGES.flatMap(s=>s.ranges.filter(r=>r.juz===parcel).map(r=>({...r,surah:s.surah}))),total=ranges.reduce((n,r)=>n+r.to-r.from+1,0),done=ranges.reduce((n,r)=>n+(pupil().trees[r.surah]?.verses||[]).filter(v=>v>=r.from&&v<=r.to).length,0);
 host.innerHTML+=`${!studentView?teacherInbox():nextStep()}<section class="cc-card" style="margin:16px 0"><h3>🌳 Tous mes arbres</h3><p>Retrouve une sourate dans toutes tes parcelles.</p><label>Choisir un arbre<select id="cc-all-trees"><option value="">Choisir une sourate…</option>${all.map(s=>`<option value="${s.id}">${esc(s.name)} · ${tree(s.id).completedAt?'Validée ✓':tree(s.id).assignment?'Passage à travailler':tree(s.id).verses.length?'En cours':'À commencer'}</option>`).join('')}</select></label></section><p class="cc-note">${studentView?'Touche un arbre pour retrouver ton passage et les messages de ton professeur.':'Vue professeur : touchez un arbre pour valider et écrire votre appréciation.'}</p><div class="cc-row"><label>Ma parcelle<select id="cc-parcel"><option value="0" ${parcel===0?'selected':''}>Tous mes arbres validés</option>${JUZ_NAMES.slice(1).map((n,i)=>`<option value="${i+1}" ${parcel===i+1?'selected':''}>Juz’ ${i+1} · ${esc(n)}</option>`).join('')}</select></label></div><p>${done} / ${total} versets validés ${parcel===0?'dans mon jardin':'dans ce Juz’'}</p><progress value="${done}" max="${total}" aria-label="Avancement du Juz’"></progress><div class="cc-tree-list">${visible.map(s=>`<button class="cc-tree-option ${hasUnreadTreeMessage(tree(s.id))?'cc-has-message':''}" data-action="tree" data-id="${s.id}">${art(s.id)}<strong>${esc(s.name)}</strong><small>${tree(s.id).verses.length}/${s.count} validés ${(tree(s.id).pendingVerses||[]).length?` · ${(tree(s.id).pendingVerses||[]).length} en attente`:``} ${due(tree(s.id))?'💧':''} ${messageBadge(s.id)}</small></button>`).join('')||'<p>Cette parcelle attend ses premières sourates. Tu peux toujours explorer le Coran.</p>'}</div><div class="cc-board" aria-label="Jardin personnel">${visible.filter(s=>tree(s.id).positions[parcel]).map(s=>{const pos=tree(s.id).positions[parcel],unread=hasUnreadTreeMessage(tree(s.id));return `<button class="cc-planted ${unread?'cc-has-message':''} ${growthMotion(s.id)&&(bloom.colorReveal||bloom.complete)?'cc-celebrate':''}" style="left:${pos.x}%;top:${pos.y}%" data-action="tree" data-id="${s.id}" ${unread?`aria-label="${esc(s.name)}, nouveau message du professeur"`:''}><span class="cc-tree-crown">${art(s.id)}</span><span>${esc(s.name)} ${due(tree(s.id))?'💧':''} </span>${messageBadge(s.id,true)}</button>`}).join('')}</div><div id="cc-detail"></div>`;
 if(parcel===0){const board=root.querySelector('.cc-board');board.className='cc-overview-garden';board.style.cssText="display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:20px;padding:32px 16px;min-height:280px;border-radius:24px;background:#c4d89c url('"+new URL('art/garden-lawn.png',base).href+"') center/cover";board.innerHTML=visible.map(s=>`<button data-action="tree" data-id="${s.id}" style="display:grid;justify-items:center;align-content:center;background:#fffef0c9;border:0"><span class="cc-tree-crown">${art(s.id)}</span><strong>${esc(s.name)}</strong><small>${tree(s.id).verses.length}/${s.count} versets validés</small>${messageBadge(s.id)}</button>`).join('')||'<p>Les arbres apparaîtront ici dès que ton professeur aura validé des versets. Retrouve tes passages à travailler dans « Tous mes arbres ».</p>';board.querySelectorAll('img').forEach(img=>{img.style.cssText='width:110px;height:135px;object-fit:contain'});root.querySelector('.cc-tree-list').hidden=true;}
 if(growthMotion(bloom?.surahId))animateProgress(root.querySelector('progress[aria-label="Avancement du Juz’"]'),done);
 root.querySelector('#cc-all-trees').onchange=e=>{const id=Number(e.target.value);if(!all.some(s=>s.id===id))return;const ranges=JUZ_RANGES.find(r=>r.surah===id).ranges;const placed=ranges.find(r=>pupil().trees[id]?.positions?.[r.juz]);parcel=(placed||ranges[0]).juz;selected=id;draw();root.querySelector('#cc-detail')?.scrollIntoView({block:'start',behavior:'auto'})};
 ...
 async function practice(id,startVerse=1,endVerse=startVerse){
  ...
  try{
   ...
   mountPhonetics(host,{surah:Number(id),base,storageKey:`${key}:phonetics:${classId}:${studentId}`});
 ...
 if(a==='attendance'){if(studentView||cloud?.studentId||!classId)return;await openAttendance(cls(),cloud?.attendance||localAttendance(key));return}
 ...
 if(a==='tab-teacher-coran'||a==='tab-teacher-class'){if(studentView||cloud?.studentId)return;studentSection=a==='tab-teacher-coran'?'teacher-coran':'garden';draw();return}
