...
import {createCloudClassroom} from '../applications/classroom/cloud-classroom.mjs?v=audit-v1';
...
async function start(){
 ...
 if(context.admin&&isAdminPage){const {mountLiveAdmin}=await import('../applications/classroom/platform-live.mjs?v=audit-v1');await mountLiveAdmin(document.querySelector('#quranPlatformRoot'),context.profile);status.textContent='';return}
 ...
 if(chosenOrg&&(context.admin||membership)){
  ...
  const {mountClassroom}=await import('../applications/classroom/classroom.mjs?v=audit-v1');await mountClassroom(document.querySelector('#classroomRoot'),{id:context.profile.id,role:'admin',full_name:context.profile.name},{cloud,classId:params.get('class-id')||undefined});status.textContent='';return;
 }
