...
async function start(){
 ...
 if(chosenStudent){if(!chosenStudent.enabled){status.textContent='Votre professeur ou l’administration doit activer la licence de cette classe.';return}const cloud=createCloudClassroom(chosenStudent.organization,{studentId:chosenStudent.id});const {mountClassroom}=await import('../applications/classroom/classroom.mjs?v=reading-options-v1');await mountClassroom(document.querySelector('#classroomRoot'),{id:context.profile.id,role:'admin',full_name:context.profile.name},{cloud,studentId:chosenStudent.id,classId:chosenStudent.classId});status.textContent='';return}
 if(chosenOrg&&(context.admin||membership)){
  ...
  home.innerHTML=`<div class="portal-topline"><span><strong>${esc(membership?.name||'Espace professeur')}</strong> · Votre espace en ligne</span><span>Classes · Jardins · Récitations</span></div>`;
  const cloud=createCloudClassroom(chosenOrg);
  const {mountClassroom}=await import('../applications/classroom/classroom.mjs?v=reading-options-v1');await mountClassroom(document.querySelector('#classroomRoot'),{id:context.profile.id,role:'admin',full_name:context.profile.name},{cloud,classId:params.get('class-id')||undefined});status.textContent='';return;
 }
