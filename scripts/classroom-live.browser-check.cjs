...
(async()=>{
 ...
 async function open(role,query){
  ...
  await context.route('**/__qa/rpc',async route=>{
   ...
   if(action==='context')result={admin:false,needsMfa:false,profile:{id:role,name:role==='teacher'?'Mme Sarah':'Maryam'},organizations:role==='teacher'?[{id:'org',name:'Ma structure',enabled:true}]:[],students:role==='student'?[{id:sid,name:'Maryam',classId:cid,organization:'org',enabled:true}]:[]};
 ...
 assert.ok(await teacher.locator('.cc-submission-audio audio').getAttribute('src'));
 ...
 const demo=await teacher.context().newPage();demo.on('pageerror',e=>errors.push(e.message));await demo.goto(origin+'/coran-licence/demonstration.html?class-id=demo-v2-class-1');await demo.locator('.cc-preview-link').first().waitFor();
 const pupilLink=await demo.locator('.cc-preview-link').first().getAttribute('href');assert.equal(new URL(pupilLink).pathname,'/coran-licence/demonstration.html');await demo.goto(pupilLink);await demo.locator('.cc-student-nav').waitFor();assert.match(await demo.locator('.cc-hero h2').innerText(),/Maryam/);
